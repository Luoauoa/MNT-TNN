...
EN_NTTNN      = 1;
EN_TBTTNN     = 1;
method_name   = {'Observed','NTTNN','TBTTNN','TNN','UTNN'};
...
if sample_ratio <= 0.05
    load('lap_mx_60min.mat');
else
    load('lap_mx_60min2.mat');
end
load('lon_filt10.mat');
X = Y;                   % transmit output key Y to X  
% X = permute(X, [2,1,1]);
...
e = 20;
A = Y_tensor0;
B = padarray(A,[e,e,e],'symmetric','both');
C = padarray(Ind,[e,e,e],'symmetric','both');
a1 = interpolate(shiftdim(B,1),shiftdim(C,1));
a1(a1<0) = 0;
a1(a1>1) = 1;
a1 = a1(e + 1:end-e,e + 1:end-e,e+1:end-e);
a1 = shiftdim(a1,2);

a2 = interpolate(shiftdim(B,2),shiftdim(C,2));
a2(a2<0) = 0;
a2(a2>1) = 1;
a2 = a2(e+1:end-e,e+1:end-e,e+1:end-e);
a2 = shiftdim(a2,1);
a = 0.5*a1+0.5*a2;
...
if EN_TBTTNN
    ...
    opts.tol = 10^-4;
    opts.max_iter = 550;  %% deserve to be fine-tuned
    opts.inner = 10;
    opts.alpha = 10;
    opts.beta  = 10;
    opts.gamma = 10;
...
%     opts.tau    = 1e-1;  
    ...
    for ii = 1:kkk
        tStart = tic;
        opts = opts_All{ii};
        [best_out, best_iter] = TC_TBTTNN3(opts, X, X, Omega, Y_tensorT);
%         L = best_out.L; E = best_out.E;
        M = best_out.M;
...
if EN_TNN
    ...
    X             = (Re_tensor{i-1} - RP.x) / RP.y;
    recovered_ind = find((X - Y_tensor0)); %% sample from the believed(high energy) region
    rs_ind       = randsample(numel(recovered_ind), round(0.8 * numel(recovered_ind)));
    new_sample  = recovered_ind(rs_ind);
    Omega_new = cat(1, Omega, new_sample);

    for ii = 1:kkk  %% kkk = 1
        tStart = tic;
        opts = opts_All{ii};
        [Re_tensor{i},~,iterations] = TC_TNN(X,Omega_new,opts);
        time(i)= toc(tStart);
        iters(i) = iterations;
        Re_tensor{i} = Re_tensor{i} .* RP.y + RP.x;
        [MPSNR_ALL(i), SSIM_ALL(i)] = quality(Y_tensor_ori, Re_tensor{i});
...
if EN_UTNN
    ...
    for ii = 1:kkk
        ...
        recovered_ind = find((X(5:25, 5:25, :) - Y_tensor0(5:25, 5:25, :))); %% sample from the believed(high energy) region
        rs_ind       = randsample(numel(recovered_ind), round(0.8 * numel(recovered_ind)));
        new_sample  = recovered_ind(rs_ind);
        Omega_new = cat(1, Omega, new_sample);
  
        [M,iterations] = TC_UTNN(U, X, Omega_new, opts);
        Re_tensor{i} = M;
        time(i)= toc(tStart);
        iters(i) = iterations;
        Re_tensor{i} = Re_tensor{i} .* RP.y + RP.x;
        [MPSNR_ALL(i), SSIM_ALL(i)] = quality(Y_tensor_ori, Re_tensor{i});
        [MAPE_ALL(i), RMSE_ALL(i)]  = metrics(Re_tensor{i}, Y_tensor_ori, Omega);
%         PSNRALL1(ii) = MPSNR_ALL(i);
%         SSIMALL1(ii) = SSIM_ALL(i);
...
if EN_TNN 
    subplot(2,4,2);imagesc(squeeze(Re_tensor{4}(:,:,342)),  'AlphaData', .8);title('TNN')
end

if EN_UTNN
    subplot(2,4,3);imagesc(squeeze(Re_tensor{5}(:,:,342)),  'AlphaData', .8);title('UTNN')
end

if EN_NTTNN
    subplot(2,4,5);imagesc(squeeze(Re_tensor{2}(:,:,342)),  'AlphaData', .8);title('NTTNN')
end

if EN_TBTTNN
    subplot(2,4,6);imagesc(squeeze(Re_tensor{3}(:,:,342)), 'AlphaData', .8);title('TBTTNN')
end

subplot(2,4,7);imagesc(squeeze(Y_tensor_ori(:,:,342)),  'AlphaData', .8);title('Original')

subplot(2,4,8);imagesc(squeeze(X0(:,:,342)),  'AlphaData', .8);title('Interpolated')
